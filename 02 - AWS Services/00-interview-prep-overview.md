# Interview Prep Overview — AWS Cloud Role

**Interview Date:** March 20, 2026, 12:00 PM IST

---

## JD Requirements → Your Resume Bridge

| JD Requirement | Your Experience | Confidence |
|---|---|---|
| ASP.NET, VB.NET | .NET/C# at eGain & Energy Exemplar | High (VB.NET is legacy .NET — same ecosystem) |
| JavaScript, React | JS/TS, Vue.js, Angular across 3 companies | High (React = component model you know from Vue) |
| AWS cloud architecture | Lambda, EC2, Redshift, Secrets Manager at eGain | High |
| Containerization | Needs prep | Medium |
| Amazon Q | Needs prep (closest: GitHub Copilot/Claude Code) | Low → prep required |
| App2Container | Needs prep | Low → prep required |
| CAST Analysis | Needs prep | Low → prep required |
| Agile/DevOps | Scrum + CI/CD pipelines mentioned | High |
| Communication & Leadership | Led analytics migration, cross-functional work | High |

---

## Files in This Pack

| File | Topic |
|---|---|
| `01-aws-cloud-architecture.md` | Core AWS services + your eGain migration story |
| `02-containerization.md` | Docker, ECS, EKS — concepts + interview answers |
| `03-amazon-q-app2container-cast.md` | The 3 niche tools from the JD |
| `04-dotnet-aspnet-vbnet.md` | Bridging your .NET C# skills to the JD |
| `05-behavioral-leadership.md` | STAR stories built from your resume |
| `06-counter-questions.md` | Tough questions + honest, resume-backed answers |

---

## 30-Minute Review Priority (Night Before)

1. **Read `03-amazon-q-app2container-cast.md` first** — highest knowledge gap
2. **Read `06-counter-questions.md`** — so you're not caught off-guard
3. **Skim `01-aws-cloud-architecture.md`** — reinforce your existing knowledge
4. **Review `05-behavioral-leadership.md`** — have 3 STAR stories ready

---

## Revised Tonight's Priority Order
1. **Read `03-amazon-q-app2container-cast.md`** — biggest gap
2. **Read `08-javascript-react.md`** → Hooks section + Vue→React table
3. **Read `07-dotnet-core-framework-csharp.md`** → async/await + DI sections
4. **Read `06-counter-questions.md`** — gap handling

## Your Strongest Talking Points

1. **SSAS → AWS Redshift Migration** — real cloud migration at scale (eGain)
2. **AWS automation** — Lambda + EC2 + Secrets Manager in production
3. **AI-assisted development** — GitHub Copilot + Claude Code = you already understand Amazon Q's value prop
4. **Full-stack versatility** — .NET backend + multiple JS frameworks
5. **CDAC + NIT Raipur** — strong academic foundation
